  ...
  for(;;) {
    ...
    while(ptr != NULL) {
      ...
      if (chunk_len) {
        ...
        ESP_LOGD(TAG, "%s", ctmp);
        strcpy(out_line.log_line, ctmp);
        if (strchr("VDIWE", ctmp[0]) && ctmp[1] == ' ' && ctmp[2] == '(') {
          strcpy(out_line.labels[0], "level");
          if (ctmp[0] == 'E') strcpy(out_line.labels[0 + LABELS_NUM], "error");
          else if (ctmp[0] == 'W') strcpy(out_line.labels[0 + LABELS_NUM], "warning");
          else if (ctmp[0] == 'I') strcpy(out_line.labels[0 + LABELS_NUM], "info");
          else if (ctmp[0] == 'D') strcpy(out_line.labels[0 + LABELS_NUM], "debug");
          else if (ctmp[0] == 'V') strcpy(out_line.labels[0 + LABELS_NUM], "verbose");
        }
